...
export async function createLiveStream(userId, title, description, category, liveMode = "camera") {
  try {
    // Production flow: request room + token together to avoid backend drift.
    videosdkTrace('S1_ROOM', 'LIVE_CREATE_START', { userId });
    const session = await createVideoSDKRoomAndToken(userId);
    const videosdkRoomId = session?.meetingId;
    const videosdkHostToken = session?.token;
    videosdkTrace('S1_ROOM', 'LIVE_CREATE_GOT_SESSION', {
      meetingId: videosdkRoomId || null,
      hasToken: Boolean(videosdkHostToken),
    });
    if (!videosdkRoomId) {
    ...
    videosdkTrace('S1_ROOM', 'LIVE_STORED', {
