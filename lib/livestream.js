...
import { databases, client, appwriteConfig, uploadFile, getPhotoUrl } from "./appwrite";
import { createVideoSDKRoomAndToken } from "./videosdkHelper";
import { scheduleCreatorSubscriberNotifications } from "./creatorNotificationDelivery";
import { validateStreamAccessPrice } from "./streamAccess";
import { LIVE_STREAM_DEFAULT_CURRENCY } from "./liveStreamSchema";
...
/** Extract missing/unknown attribute name from Appwrite createDocument errors. */
function parseAppwriteMissingAttribute(error) {
  const raw = String(error?.message || error || "");
  const patterns = [
    /Unknown attribute[:\s]+["']?(\w+)["']?/i,
    /Attribute not found[:\s]+["']?(\w+)["']?/i,
    /Invalid document structure[^"']*["'](\w+)["']/i,
...
const LIVE_STREAM_OPTIONAL_ATTRS = new Set([
  "liveMode",
  "thumbnail",
  "description",
  // Paid streaming (Step 1 schema — stripped until attributes exist in Appwrite)
  "isPaid",
  "price",
  "currency",
]);

const LIVE_STREAM_ATTR_HINTS = {
  videosdkRoomId: 'String — VideoSDK meeting room ID (required for all live streams)',
  liveMode: 'String — enum: camera | screen',
  hostId: 'String',
  hostUsername: 'String',
  hostAvatar: 'String',
  title: 'String',
  ...
  endTime: 'DateTime',
  thumbnail: 'String',
  isPaid: 'Boolean — true when viewers must purchase access',
  price: 'Float — ticket price in dollars when isPaid is true',
  currency: 'String — ISO currency code (e.g. USD)',
};

function formatMissingAttributeError(attribute) {
  const hint = LIVE_STREAM_ATTR_HINTS[attribute] || 'see Appwrite collection schema';
  let message =
    `Appwrite liveStreams collection is missing attribute "${attribute}". ` +
    `Add it in Appwrite Console → Database → liveStreams → Attributes. Type: ${hint}.`;

  if (attribute === 'videosdkRoomId') {
    message +=
      ' This attribute is required for VideoSDK live streaming and must be added before you can go live.';
  }

  return message;
}

async function createLiveStreamDocument(streamDocId, payload) {
  let current = { ...payload };
  const stripped = [];
  const maxAttempts = LIVE_STREAM_OPTIONAL_ATTRS.size + 2;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      const missing = parseAppwriteMissingAttribute(error);
      if (!missing) throw error;

      if (missing === 'videosdkRoomId' || !LIVE_STREAM_OPTIONAL_ATTRS.has(missing)) {
        throw new Error(formatMissingAttributeError(missing));
      }

      if (!(missing in current)) throw error;
  ...
  throw new Error(
    `Failed to create live stream document after removing optional fields: ${stripped.join(', ')}`
  );
}
...
/** Unique viewer participant id per stream — avoids stale VideoSDK sessions when rejoining. */
export function buildLiveViewerParticipantId(streamDocId, userId) {
  const sid = String(streamDocId || '')
    .replace(/[^a-zA-Z0-9]/g, '')
    .slice(0, 16);
  const uid = String(userId || '')
    .replace(/[^a-zA-Z0-9]/g, '')
    .slice(0, 16);
  return `viewer-${sid || 's'}-${uid || 'u'}`;
}
...
export async function createLiveStream(
  ...
  try {
    ...
    const wantsPaid = options?.isPaid === true;
    let paidPrice = 0;
    let paidCurrency = LIVE_STREAM_DEFAULT_CURRENCY;
    if (wantsPaid) {
      const priceCheck = validateStreamAccessPrice(options?.price);
      if (!priceCheck.ok) {
        throw new Error(priceCheck.error);
      }
      paidPrice = priceCheck.price;
      paidCurrency = String(options?.currency || LIVE_STREAM_DEFAULT_CURRENCY)
        .trim()
        .toUpperCase() || LIVE_STREAM_DEFAULT_CURRENCY;
    }
    ...
    if (wantsPaid) {
      streamPayload.price = paidPrice;
      streamPayload.currency = paidCurrency;
    }

    const liveStream = await createLiveStreamDocument(streamDocId, streamPayload);
    ...
    if (rawMessage.includes('Appwrite liveStreams collection is missing attribute')) {
      throw error;
    }
