...
const PAID_STREAM_ATTRS = ["isPaid", "price", "currency"];

function stripPaidStreamFields(payload) {
  const next = { ...payload };
  PAID_STREAM_ATTRS.forEach((key) => {
    delete next[key];
  });
  return next;
}
...
const LIVE_STREAM_ATTR_HINTS = {
  ...
  isPaid: 'Boolean — true for paid streams',
  price: 'Float — ticket price when isPaid is true',
...
function formatMissingAttributeError(attribute, isPaidAttempt) {
  ...
  } else if (isPaidAttempt && ['isPaid', 'price', 'currency'].includes(attribute)) {
    message += ' See docs/paid-live-streaming.md for paid stream setup.';
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      if (missing === 'videosdkRoomId' || !LIVE_STREAM_OPTIONAL_ATTRS.has(missing)) {
        throw new Error(
          formatMissingAttributeError(missing, Boolean(payload.isPaid))
        );
      }

      if (!(missing in current)) throw error;

      if (PAID_STREAM_ATTRS.includes(missing)) {
        current = stripPaidStreamFields(current);
        stripped.push(...PAID_STREAM_ATTRS.filter((key) => key in payload));
      } else {
...
export async function createLiveStream(
  ...
  try {
    ...
    const price =
      isPaid && Number.isFinite(parsedPrice) && parsedPrice > 0 ? parsedPrice : 0;
    ...
    if (isPaid && price > 0) {
      streamPayload.isPaid = true;
      streamPayload.price = price;
      streamPayload.currency = currency;
    }
