...
if (tokenHostIsAppwrite && tokenPath !== "") {
  console.warn(
    "⚠️ VideoSDK token host is Appwrite; token path must be empty. Set EXPO_PUBLIC_VIDEOSDK_TOKEN_PATH=''."
  );
}
if (!tokenHostIsAppwrite && tokenPath !== "/get-token") {
  console.warn(
    "⚠️ VideoSDK token host is not Appwrite; token path should be '/get-token'. Set EXPO_PUBLIC_VIDEOSDK_TOKEN_PATH=/get-token."
  );
}
