    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403 && data?.error === 'Payment required') {
          error.code = 'STREAM_PAYMENT_REQUIRED';
        }
        throw error;
      }
...
function buildTokenUrl(roomId, participantId, extraQuery = {}) {
  ...
  if (extraQuery.accessUserId) {
    query.userId = String(extraQuery.accessUserId);
    delete query.accessUserId;
  }
  return buildUrl(VIDEOSDK_CONFIG.tokenServerUrl, VIDEOSDK_CONFIG.tokenPath, query);
}
...
export async function createVideoSDKRoomAndToken(participantId) {
  ...
  videosdkTrace('S1_ROOM', 'START', { url, participantId: participantId || null });
  try {
    const data = await requestJsonWithRetry({
 ...
 * @param {{ purpose?: 'viewer' | 'live' | 'call', streamId?: string, accessUserId?: string }} [options] - `live`/`viewer` = interactive live; `accessUserId` = Appwrite user id for paid-stream entitlement checks
...
export async function getVideoSDKToken(roomId, participantId, options = {}) {
  ...
  const extraQuery = {};
  if (options?.purpose === "viewer" || options?.purpose === "live") {
    extraQuery.purpose = options.purpose === "live" ? "live" : "viewer";
  }
  if (options?.streamId) {
    extraQuery.streamId = String(options.streamId);
  }
  if (options?.accessUserId) {
    extraQuery.accessUserId = String(options.accessUserId);
  }
  ...
  try {
    ...
    if (error?.code === "STREAM_PAYMENT_REQUIRED" || error?.status === 403) {
      throw new Error(
        "Payment required. Complete your purchase on the paywall before joining this stream."
      );
    }
    ...
    throw new Error(`Token request failed (${url}): ${error?.message || String(error)}`);
  }
}

/**
 * Poll until useMeeting().join is bound (MeetingProvider child mounted).
 * @param {() => (() => void)|null|undefined} getJoin
 * @param {{ timeoutMs?: number, intervalMs?: number, isCancelled?: () => boolean }} [options]
 * @returns {Promise<(() => void)|null>}
 */
export async function waitForMeetingJoinFn(getJoin, options = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    const join = typeof getJoin === "function" ? getJoin() : null;
