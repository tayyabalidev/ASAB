...
const ROOM_CREATE_TIMEOUT_MS = 45000;
const ROOM_CREATE_MAX_ATTEMPTS = 2;
const ROOM_CREATE_RETRY_DELAY_MS = 1200;
...
async function createRoomViaBackend() {
  ...
  let lastError = null;
  for (let attempt = 1; attempt <= ROOM_CREATE_MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (__DEV__) {
        console.warn("[VideoSDK] room create attempt failed", {
          attempt,
          maxAttempts: ROOM_CREATE_MAX_ATTEMPTS,
          url,
          reason: isAbort ? "timeout" : error?.message || String(error),
        });
      }
      if (attempt < ROOM_CREATE_MAX_ATTEMPTS) {
        await new Promise((resolve) => setTimeout(resolve, ROOM_CREATE_RETRY_DELAY_MS));
      }
    } finally {
      clearTimeout(t);
    }
  }
  throw lastError || new Error("Room creation failed");
}
    ...
    if (e?.name === "AbortError") {
      throw new Error(
        `Room creation timed out after ${Math.round(
          ROOM_CREATE_TIMEOUT_MS / 1000
        )}s (retried ${ROOM_CREATE_MAX_ATTEMPTS}x). Check Appwrite function cold start, execution logs, and network.`
      );
    }
    throw new Error(
