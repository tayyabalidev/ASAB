...
export async function getVideoSDKToken(meetingId) {
  try {
    ...
    // Without tokenServerUrl, MeetingProvider falls back to apiKey (see VideoSDKCall.jsx).
    // That is only suitable for local testing; release builds should use JWTs from your server.
    if (!__DEV__) {
      console.warn(
        '[VideoSDK] Release build has no tokenServerUrl. Configure a backend that mints VideoSDK JWTs and set VIDEOSDK_CONFIG.tokenServerUrl — client API key fallback may fail or be rejected.'
      );
    } else {
      console.log('Using VideoSDK API key directly (__DEV__ only). Token optional.');
    }
    return null;
  } catch (error) {
