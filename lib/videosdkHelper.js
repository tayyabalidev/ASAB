 ...
 * @param {{ purpose?: 'viewer' | 'live' | 'call', streamId?: string, userId?: string, accessUserId?: string }} [options]
...
export async function getVideoSDKToken(roomId, participantId, options = {}) {
  ...
  if (options?.streamId) {
    extraQuery.streamId = String(options.streamId).trim();
  }
  const accessUserId = options?.userId || options?.accessUserId;
  if (accessUserId) {
    extraQuery.userId = String(accessUserId).trim();
  }
