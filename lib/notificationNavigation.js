...
export function navigateToHomePost(router, postId) {
  ...
  if (!router || !id) return;
  try {
    router.replace({ pathname: '/(tabs)/home', params: { postId: id } });
  } catch (_) {
...
export function navigateFromPushData(router, rawData) {
  ...
  if (
    ...
    safeRouterNavigate(router, (r) => {
      r.push({
        pathname: '/live-map',
        params: fromUserId ? { partnerId: fromUserId } : {},
      });
    });
    return true;
  }
...
export function navigateFromInboxNotification(router, notification) {
  ...
  if (type === 'location_invite' || type === 'location_share' || type === 'location_nearby') {
    safeRouterNavigate(router, (r) => {
      r.push({
        pathname: '/live-map',
        params: fromUserId ? { partnerId: fromUserId } : {},
