...
import {
  getCallDocumentRealtimeChannel,
  getCallsRealtimeChannel,
...
/**
 * Realtime + polling listener for new incoming calls (receiver only).
 * @param {string} userId
 * @param {(call: object) => void} onIncoming
...
export function subscribeIncomingCalls(userId, onIncoming) {
  ...
  const emitIfNew = (call) => {
    if (disposed || !call?.$id || call.receiverId !== userId) return;
    if (call.status !== CallState.CALLING) return;
    if (call.$id === lastEmittedId) return;
    lastEmittedId = call.$id;
    onIncoming(call);
  };
  ...
  if (isAppwriteRealtimeEnabled()) {
    const channel = getCallsRealtimeChannel();
    realtimeUnsub = registerAppwriteRealtimeChannel(channel, (response) => {
      const payload = response?.payload;
      if (!payload || payload.receiverId !== userId) return;
      if (payload.status === CallState.CALLING) {
        emitIfNew(payload);
      } else if (payload.$id === lastEmittedId) {
        lastEmittedId = '';
      }
    });
  }
...
export function subscribeCallUpdates(callId, callback) {
  ...
  let realtimeUnsub = null;
  if (isAppwriteRealtimeEnabled()) {
    const channel = getCallDocumentRealtimeChannel(callId);
    if (channel) {
  ...
  const intervalId = setInterval(fetchAndEmit, 800);
