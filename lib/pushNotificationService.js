...
/**
 * Deliver push to user IDs — direct Expo when tokens are readable, else server relay.
 */
async function deliverPushToUsers({ toUserIds, title, body, channelId, data }) {
  ...
  await relayPushNotification({ toUserIds: recipients, title, body, channelId, data });
}
