...
export async function processAdvertisingPayment(stripe, amount, advertiserId, subscriptionPlan) {
  try {
    ...
    if (confirmation.success) {
      console.log('✅ Advertising payment verified successfully on server');
      return {
        success: true,
        paymentIntentId: paymentIntent.paymentIntentId,
        amount: confirmation.amount || amount,
      ...
      console.error('❌ Advertising payment verification failed:', confirmation.message || confirmation.status);
      throw new Error(confirmation.message || `Payment status: ${confirmation.status || 'unknown'}`);
    }
  } catch (error) {
    console.error('Error processing advertising payment:', error);
    throw error;
  }
}