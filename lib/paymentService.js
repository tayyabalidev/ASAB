 ...
 */
export async function createStreamAccessPaymentIntent(
  amount,
  buyerId,
  hostId,
  streamId,
  ...
  try {
    ...
    const response = await fetch(`${PROCESSING_SERVER_URL}/api/create-stream-access-payment-intent`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    throw new Error(`Failed to create stream access payment intent: ${error.message}`);
  }
}

/**
 * Process paid live stream access payment with Stripe SDK
 */
export async function processStreamAccessPayment(
  stripe,
  amount,
  buyerId,
  hostId,
  ...
  const paymentIntent = await createStreamAccessPaymentIntent(
    amount,
    buyerId,
    hostId,
    streamId,
...
/**
 * Verify stream access via processing server (used before token fetch)
 */
export async function verifyStreamAccessOnServer(streamId, userId) {
  const serverReachable = await isServerReachable(PROCESSING_SERVER_URL).catch(() => false);
  if (!serverReachable) {
    return { allowed: true, reason: 'server_unreachable' };
  }

  const params = new URLSearchParams({ streamId, userId });
  const response = await fetch(`${PROCESSING_SERVER_URL}/api/check-stream-access?${params.toString()}`);
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.error || `Access check failed (${response.status})`);
  }
  return response.json();
}