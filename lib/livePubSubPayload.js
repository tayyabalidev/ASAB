  ...
  if (raw != null && typeof raw === 'object' && !Array.isArray(raw)) {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      try {
        ...
        if (trimmed === 'SEND_AND_RECV' || trimmed === 'RECV_ONLY' || trimmed === 'SIGNALLING_ONLY') {
          body = { mode: trimmed };
        }
      }
    } else if (
      trimmed === 'SEND_AND_RECV' ||
      trimmed === 'RECV_ONLY' ||
      trimmed === 'SIGNALLING_ONLY'
    ) {
      body = { mode: trimmed };
    }
  }
...
/**
 * Publish a stage/control command. Awaits publish so callers don't unmount early.
 * Prefer broadcast (no sendOnly) — sendOnly has dropped invites on some SDK builds.
 */
export async function publishStageCommand(publish, payload, { sendOnly } = {}) {
  ...
  // Only use sendOnly when explicitly requested; default is broadcast + client-side filter.
  if (Array.isArray(sendOnly) && sendOnly.length) {
    options.sendOnly = sendOnly.map(String);
  }

  // 1) Official ILS guides often publish a plain object `{ mode }`.
  try {
  ...
  // 2) Typed API: message must be a string.
  try {
  ...
  // 3) Newer API shape: string message + payload object.
