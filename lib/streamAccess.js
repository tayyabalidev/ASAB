...
export async function hasStreamAccess(stream, userId) {
  ...
  try {
    ...
    if (serverResult?.allowed === true) return true;
    if (
      serverResult?.allowed === false &&
      String(serverResult?.reason || "").toLowerCase() === "payment_required"
    ) {
      return false;
    }
  } catch (_) {
    /* fall through to client-side purchase lookup */
  }
