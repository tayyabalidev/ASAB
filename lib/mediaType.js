...
export const inferMediaKindFromUrl = (url) => {
  ...
  // Video extensions (incl. HLS master playlist for Mux / CDN)
  if (/\.(mp4|mov|m4v|webm|mkv|3gp|m3u8)$/i.test(u)) return "video";
...
export const isMuxPlaceholderVideo = (url) => isMuxPendingVideoUrl(url);

export const isVideoMedia = (url, hint) => {
  if (!url) return false;
  if (hint === "photo" || hint === "image") return false;
  if (hint === "video") return true;
  if (isMuxPlaceholderVideo(url)) return true;
