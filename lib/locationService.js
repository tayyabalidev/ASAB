...
export function canViewerSeeLocationDoc(doc, viewerId, mutualFriendIds) {
  ...
  if (mode === LOCATION_PRIVACY_MODES.FRIENDS) {
    return (mutualFriendIds || []).map(String).includes(ownerId);
  }
...
/**
 * Load locations visible to the viewer (mutual-friend “Friends” mode + everyone/selected).
 */
export async function fetchVisibleFriendLocations(viewerUser) {
  const viewerId = viewerUser?.$id;
  if (!viewerId || !collectionId()) return [];

  const mutualFriendIds = getMutualFriendIds(viewerUser);
  ...
  const visible = documents.filter((doc) =>
    canViewerSeeLocationDoc(doc, viewerId, mutualFriendIds)
  );
