...
function inferMimeTypeFromFile(file, uploadType) {
  ...
  if (/\.(mp4|m4v|mov|qt|3gp)(\?|#|$)/.test(uriOrName)) return "video/mp4";
...
export async function uploadFile(file, type) {
  ...
  if (type === 'video' && mimeType && (mimeType.includes('mov') || mimeType.includes('quicktime'))) {
...
export async function getFilePreview(fileId, type) {
  try {
    ...
    if (type === "video") {
      // Use download endpoint for iOS/AVPlayer reliability.
      const baseUrl = `${appwriteConfig.endpoint}/storage/buckets/${appwriteConfig.storageId}/files/${fileId}`;
      fileUrl = `${baseUrl}/download?project=${appwriteConfig.projectId}`;
    } else if (type === "image") {
