...
import { enableCreatorNotificationsOnFollow, disableCreatorNotificationsOnUnfollow } from "./creatorSubscriptions";
import {
  isMuxPendingVideoUrl,
  MUX_VIDEO_PENDING_PLACEHOLDER_URL,
} from "./muxConfig";
import { sendActivityPushNotification } from "./pushNotificationService";
import { extractExtraPhotoRaw, mergeExtraPhotoIdsIntoEdits } from "./photoSlides";

export const appwriteConfig = {
  endpoint: "https://nyc.cloud.appwrite.io/v1",
  platform: "com.bilal.asab",
  ...
  likesCollectionId: "699e9124001af0a247cd", // Likes Collection - Replace with actual ID from Appwrite
  notificationsCollectionId: "6889d903000dd865f451",
  // Live Streams — add string attribute `liveMode` (e.g. enum: camera | screen) for stream type badges.
  // Paid streams: Boolean `isPaid`, Float `price`, String `currency` — see lib/liveStreamSchema.js
  liveStreamsCollectionId: "68f20f1f00332e083aff", // Live Streams ✅
  // streamPurchases — set EXPO_PUBLIC_STREAM_PURCHASES_COLLECTION_ID after creating collection in Appwrite
  streamPurchasesCollectionId:
    (typeof process !== "undefined" &&
      process.env?.EXPO_PUBLIC_STREAM_PURCHASES_COLLECTION_ID &&
      String(process.env.EXPO_PUBLIC_STREAM_PURCHASES_COLLECTION_ID).trim()) ||
    "",
  ...
  callsCollectionId: "68a2e18e0024c20ecd40", // Calls Collection - Replace with actual ID from Appwrite
  ...
  // Friends Live Location — Appwrite collection `userLocations` (see docs/FRIENDS_LIVE_LOCATION_BACKEND.md)
  userLocationsCollectionId: "6a74d4660024fc37b862",
  // Map moments (photos on Friends Live Map) — create collection then set ID / env
  mapMomentsCollectionId:
    (typeof process !== "undefined" &&
...
/** Shared Appwrite client (session, realtime). Used by lib/calls.js for subscriptions. */
export { client };

// iOS Video URL Helper
export function getIOSCompatibleVideoUrl(videoUrl) {
  const candidates = getVideoPlaybackUrls(videoUrl);
  return candidates[0] || videoUrl || null;
}

function stripPlaybackUrlHash(videoUrl) {
  if (!videoUrl || typeof videoUrl !== "string") return videoUrl;
  return videoUrl.replace(/#advideo$/i, "");
}

// Return ordered playback URL candidates for resilient video loading.
export function getVideoPlaybackUrls(videoUrl) {
  if (!videoUrl) return [];
  if (isMuxPendingVideoUrl(videoUrl)) return [];

  try {
    const raw = stripPlaybackUrlHash(videoUrl);
    ...
    const isAppwriteStorageUrl =
      url.pathname.includes('/storage/buckets/') && url.pathname.includes('/files/');

    if (!isAppwriteStorageUrl) {
      return [base];
    }

    const viewUrl = base.replace('/download', '/view');
    const downloadUrl = base.replace('/view', '/download');
    const candidates = [viewUrl, downloadUrl, base].filter(Boolean);
    return [...new Set(candidates)];
  } catch (error) {
    return [stripPlaybackUrlHash(videoUrl) || videoUrl];
  }
}

/**
 * Returns a poster URI only when it is not the same Appwrite file as the video.
 * Using the video view URL as an Image/expo-av poster breaks loading and hurts performance.
 */
export function getVideoPosterUri(thumbnailUrl, videoUrl) {
  if (thumbnailUrl == null || thumbnailUrl === "") return undefined;
  const t = String(thumbnailUrl).trim();
  if (!t) return undefined;

  const fileIdFromUrl = (u) => {
    if (!u || typeof u !== "string") return null;
    const m = u.match(/\/files\/([^/?]+)/);
    return m ? m[1] : null;
  };

  const videoFileId = videoUrl ? fileIdFromUrl(String(videoUrl)) : null;
  const thumbFileId =
    fileIdFromUrl(t) ||
    (t.length < 120 && !t.includes("/") && !t.startsWith("http") ? t : null);

  if (videoFileId && thumbFileId && thumbFileId === videoFileId) {
    return undefined;
  }
  ...
  if (!t.startsWith("http")) {
    if (thumbFileId) {
      thumbDisplay = `${appwriteConfig.endpoint}/storage/buckets/${appwriteConfig.storageId}/files/${thumbFileId}/preview?width=800&height=800&gravity=center&quality=85&project=${appwriteConfig.projectId}`;
    } else {
      return undefined;
    }
  }
  ...
  if (videoUrl) {
    ...
    if (stripQuery(thumbDisplay) === stripQuery(v)) return undefined;
  }

  return thumbDisplay;
}

/** Home / profile feeds: fetch enough pages so we are not stuck at Appwrite's default ~25 row cap. */
const FEED_MAX_DOCUMENTS = 150;
const FEED_PAGE_SIZE = 50;
const TRENDING_LATEST_LIMIT = 50;
const TRENDING_POOL_SIZE = 150;
const TRENDING_RESULT_LIMIT = 50;
const USER_POSTS_MAX_DOCUMENTS = 200;
...
function getTrendingVideoScore(post, nowMs = Date.now()) {
  ...
  const recencyBoost = Math.max(0, 168 - ageHours) * 2.5;
  return recencyBoost + getPostEngagementScore(post);
}

async function fetchLatestDocuments(collectionId, options = {}) {
  const {
    maxTotal = FEED_MAX_DOCUMENTS,
  ...
  while (results.length < maxTotal) {
    const limit = Math.min(pageSize, maxTotal - results.length);
    const queries = [
      ...filterQueries,
...
function inferMimeTypeFromFile(file, uploadType) {
  const explicitType = file?.mimeType || file?.type;
  if (explicitType && typeof explicitType === "string" && explicitType.includes("/")) {
    return explicitType;
  }

  const uriOrName = `${file?.uri || ""} ${file?.name || ""}`.toLowerCase();

  if (/\.(mp4)(\?|#|$)/.test(uriOrName)) return "video/mp4";
  if (/\.(m4v)(\?|#|$)/.test(uriOrName)) return "video/x-m4v";
  if (/\.(mov|qt)(\?|#|$)/.test(uriOrName)) return "video/quicktime";
  if (/\.(3gp)(\?|#|$)/.test(uriOrName)) return "video/3gpp";
  if (/\.(webm)(\?|#|$)/.test(uriOrName)) return "video/webm";
  if (/\.(jpg|jpeg)(\?|#|$)/.test(uriOrName)) return "image/jpeg";
  if (/\.(png)(\?|#|$)/.test(uriOrName)) return "image/png";
  if (/\.(webp)(\?|#|$)/.test(uriOrName)) return "image/webp";
  if (/\.(heic|heif)(\?|#|$)/.test(uriOrName)) return "image/heic";
  if (/\.(mp3|m4a|aac|wav)(\?|#|$)/.test(uriOrName)) return "audio/mpeg";
...
// Register user
export async function createUser(email, password, username, birthday) {
  try {
...
export async function deleteAccount() {
  try {
    ...
    if (currentUser && currentUser.documents.length > 0) {
      ...
      // 1. Delete user's posts (videos and photos)
      try {
      ...
      try {
        ...
        for (const photo of userPhotos.documents) {
          try {
            await databases.deleteDocument(
              appwriteConfig.databaseId,
              appwriteConfig.photoCollectionId,
    ...
    // 4. Delete the Appwrite account (this permanently deletes the account)
    // Note: Appwrite doesn't have a direct account.delete() method
    // We delete all user data and sessions, effectively making the account unusable
    // For OAuth accounts, we can't delete the identity, but we delete all app data
    ...
    // For email/password accounts, we can try to delete the account
    // For OAuth accounts (Google/Facebook), the identity remains but all app data is deleted
    try {
      // Try to delete the account if it's an email/password account
      if (currentAccount.email) {
        // Note: Appwrite SDK might not have account.delete() - this depends on your Appwrite version
        // If available, uncomment: await account.delete();
      }
    } catch (e) {
      // If account deletion fails, the user data is already deleted above
    }
...
export async function uploadFile(file, type) {
  ...
  // iOS pickers may omit type/mimeType; infer a safe value.
  const mimeType = inferMimeTypeFromFile(file, type);
  
  // Ensure Appwrite-compatible file name
  let compatibleFileName = ensureAppwriteCompatibleFileName(file.name, mimeType);
  
  // Preserve real container/mime type for videos. Renaming/translating
  // MOV/HEVC files to MP4 metadata can break decoding for long videos.
  const finalMimeType = mimeType;
  // If picker did not provide a file name/extension, provide a safe one.
  if (!/\.[a-z0-9]+$/i.test(compatibleFileName)) {
    if (type === "video") compatibleFileName = `${compatibleFileName}.mp4`;
    else if (type === "image") compatibleFileName = `${compatibleFileName}.jpg`;
    else if (type === "audio") compatibleFileName = `${compatibleFileName}.mp3`;
  }
  ...
  try {
    const uploadedFile = await retryNetworkRequest(async () => {
      ...
      return result;
    }, type === "video" ? 5 : 4, type === "video" ? 1500 : 1000);
...
export async function getFilePreview(fileId, type) {
  try {
    ...
    if (type === "video") {
      // Use view endpoint so the player can stream with range requests.
...
export async function createVideoPost(form) {
  try {
    ...
    const thumbnailAsset =
      form.thumbnail &&
      typeof form.thumbnail === "object" &&
      typeof form.thumbnail.uri === "string" &&
      form.thumbnail.uri.trim() !== ""
        ? form.thumbnail
        : null;

    const [videoUploadResult, thumbnailUploadResult, musicUploadResult] =
      await Promise.all([
        uploadFile(form.video, "video"),
        thumbnailAsset
          ? uploadFile(thumbnailAsset, "image").catch(() => null)
          : Promise.resolve(null),
        form.music
          ? uploadFile(form.music, "audio").catch(() => null)
          : Promise.resolve(null),
      ]);

    videoUrl = videoUploadResult;
    thumbnailUrl = thumbnailUploadResult;
    musicUrl = musicUploadResult;
    ...
    // Add optional/custom attributes only when explicitly provided.
    if (form.visibility) {
      documentData.visibility = form.visibility;
    }
    ...
    const createWithUnknownAttributeRetry = async (payload) => {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const unknownAttrMatch = message.match(/Unknown attribute:?\s*"?([a-zA-Z0-9_.$-]+)"?/i);
          const unknownAttr = unknownAttrMatch?.[1];
          if (!isUnknownAttr || !unknownAttr || !(unknownAttr in attemptPayload)) {
            throw err;
          }
          // Remove unsupported schema field and retry.
          delete attemptPayload[unknownAttr];
        }
      }
      throw new Error("Unable to create video post after removing unsupported attributes.");
    };

    // Create document with schema-adaptive retry.
    let newPost;
    try {
      newPost = await createWithUnknownAttributeRetry(documentData);
    } catch (createError) {
      // Fallback with minimal fields, then adaptively strip unknowns if needed.
      if (createError.message && createError.message.includes("Unknown attribute")) {
        ...
        if (form.visibility) {
          basicData.visibility = form.visibility;
        }
        newPost = await createWithUnknownAttributeRetry(basicData);
      } else {
        throw createError;
      }
    }
    ...
    if (error.message && error.message.includes("Missing required attribute")) {
      throw new Error(
        `Video collection schema requires extra fields: ${error.message}. ` +
        `Please ensure required fields for video include at least title, thumbnail, video, creator and any custom required fields in your Appwrite schema.`
      );
    }
...
/** Appwrite URL attributes reject ""; use a real https URL until Mux webhook sets image.mux.com thumbnail. */
const PENDING_MUX_THUMBNAIL_PLACEHOLDER =
  "https://placehold.co/720x1280/111111/888888/png?text=Processing";
...
/**
 * Create a video post row before uploading the file to Mux (video = URL placeholder until webhook).
 * Add optional Appwrite attributes: mux_status, mux_playback_id, mux_asset_id, mux_upload_id, duration.
 */
export async function createMuxVideoPostDocument(form) {
  try {
    let thumbnailUrl = null;
    let musicUrl = null;
    ...
    const [thumbnailUploadResult, musicUploadResult] = await Promise.all([
      thumbnailAsset
        ? uploadFile(thumbnailAsset, "image").catch(() => null)
        : Promise.resolve(null),
      form.music
    ...
    const thumbnailForDoc = isAbsoluteHttpUrl(thumbnailUrl)
      ? thumbnailUrl.trim()
      : PENDING_MUX_THUMBNAIL_PLACEHOLDER;

    const documentData = {
      title: form.title,
      thumbnail: thumbnailForDoc,
      video: MUX_VIDEO_PENDING_PLACEHOLDER_URL,
      prompt: form.prompt,
      creator: form.userId,
    ...
    if (form.edits) {
      documentData.edits =
        typeof form.edits === "string" ? form.edits : JSON.stringify(form.edits);
    }
    ...
    const createWithUnknownAttributeRetry = async (payload) => {
      ...
      throw new Error("Unable to create Mux video post after schema retries.");
    };
    ...
    try {
      ...
      if (
        createError.message &&
        createError.message.includes("Unknown attribute")
      ) {
        const basicData = {
          title: form.title,
          thumbnail: thumbnailForDoc,
    ...
    throw new Error(`Failed to create Mux video post: ${error.message}`);
  }
}
...
export async function getAllPosts() {
  try {
    const documents = await fetchLatestDocuments(
      appwriteConfig.videoCollectionId,
      { maxTotal: FEED_MAX_DOCUMENTS, pageSize: FEED_PAGE_SIZE }
    );
...
export async function getUserPosts(userId) {
  try {
    const documents = await fetchLatestDocuments(
      appwriteConfig.videoCollectionId,
      {
        maxTotal: USER_POSTS_MAX_DOCUMENTS,
        pageSize: FEED_PAGE_SIZE,
        filterQueries: [Query.equal("creator", userId)],
      }
    );
...
export async function getLatestPosts() {
  try {
    const posts = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.videoCollectionId,
      [Query.orderDesc("$createdAt"), Query.limit(TRENDING_LATEST_LIMIT)]
    );
...
/** Top N newest videos ranked by recency + engagement (likes, comments, shares, views). */
export async function getTrendingVideos() {
  try {
    const documents = await fetchLatestDocuments(appwriteConfig.videoCollectionId, {
      maxTotal: TRENDING_POOL_SIZE,
      pageSize: FEED_PAGE_SIZE,
    });
    const nowMs = Date.now();

    return documents
      .map((post) => ({ ...post, postType: "video" }))
      .sort((a, b) => {
        const scoreDiff = getTrendingVideoScore(b, nowMs) - getTrendingVideoScore(a, nowMs);
...
export async function getFollowingPosts(userId) {
  try {
    ...
    const fetchCreatorDocs = async (collectionId) => {
      const docs = [];
      for (let i = 0; i < uniqueIds.length; i += chunkSize) {
        ...
        try {
          ...
          for (const fid of chunk) {
            try {
              const one = await databases.listDocuments(
                appwriteConfig.databaseId,
                collectionId,
                [
                  Query.equal("creator", fid),
                  Query.orderDesc("$createdAt"),
                  Query.limit(40),
                ]
              );
              docs.push(...(one.documents || []));
            } catch {
              // skip failed creator
            }
          }
    ...
    const [videoDocs, photoDocs] = await Promise.all([
      fetchCreatorDocs(appwriteConfig.videoCollectionId),
      fetchCreatorDocs(appwriteConfig.photoCollectionId).catch(() => []),
    ]);

    const byId = new Map();
    for (const doc of videoDocs) {
      if (doc?.$id) byId.set(`v:${doc.$id}`, { ...doc, postType: "video" });
    }
    for (const doc of photoDocs) {
      const hydrated = hydratePhotoPost(doc, { postType: "photo" });
      if (hydrated?.$id) byId.set(`p:${hydrated.$id}`, hydrated);
    }
    ...
    return merged.slice(0, FEED_MAX_DOCUMENTS);
  } catch (error) {
...
export function getPhotoUrl(photoField) {
  ...
  // Keep non-Appwrite URLs (e.g. ui-avatars) as-is.
  if (photoString.startsWith("http") && !photoString.includes("/files/")) {
    return photoString;
  }

  const fileId = photoString.startsWith("http")
    ? extractFileIdFromUrl(photoString)
    : photoString;

  if (!fileId) {
    return photoString.startsWith("http") ? photoString : null;
  }

  // Use /view so portrait photos are not square-cropped (preview+gravity=top
  // was cutting off the bottom of faces).
...
function stripLocalEditsUris(editsPayload) {
  if (!editsPayload || typeof editsPayload !== "object") return editsPayload || {};
  const out = { ...editsPayload };
  const keepRemote = (uri) => {
  ...
  if (Array.isArray(out.i)) {
    out.i = out.i
      .map((overlay) => ({ ...overlay, u: keepRemote(overlay?.u) }))
      .filter((overlay) => overlay.u);
    if (!out.i.length) delete out.i;
  }
  if (Array.isArray(out.imageOverlays)) {
    out.imageOverlays = out.imageOverlays
      .map((overlay) => ({ ...overlay, uri: keepRemote(overlay?.uri) }))
      .filter((overlay) => overlay.uri);
    if (!out.imageOverlays.length) delete out.imageOverlays;
  }
  return out;
}

function hydratePhotoPost(post, extra = {}) {
  if (!post) return null;
  const photoUrl = getPhotoUrl(post.photo);
  if (!photoUrl || typeof photoUrl !== "string") return null;

  const extraUrls = extractExtraPhotoRaw(post)
    .map((field) => getPhotoUrl(field))
    .filter((uri) => typeof uri === "string" && uri.trim());

  const photos = [photoUrl];
  const seen = new Set([photoUrl]);
  extraUrls.forEach((uri) => {
    if (!seen.has(uri)) {
      seen.add(uri);
      photos.push(uri);
    }
...
export async function createPhotoPost(form) {
  try {
    ...
    const extraFiles = Array.isArray(form.extraPhotos) ? form.extraPhotos : [];
    for (const extraFile of extraFiles) {
      if (!extraFile) continue;
      const extraUrl = await uploadFile(extraFile, "image");
      if (!extraUrl) {
        throw new Error("Failed to upload one of the slide photos");
      }
      extraPhotoFields.push(extractFileIdFromUrl(extraUrl) || extraUrl);
    }
    ...
    const documentData = {
      photo: photoField,
      creator: form.userId,
      // Some Appwrite schemas use a required custom createdAt field.
      createdAt: nowIso,
      // Your collection also has this required datetime column.
      createPhotoPost: nowIso,
      // Required enum in your schema; default to public when not provided.
    ...
    const editsPayload = stripLocalEditsUris(
      form.edits && typeof form.edits === "object"
        ? { ...form.edits }
        : form.edits && typeof form.edits === "string"
          ? (() => {
              try {
                return JSON.parse(form.edits);
              } catch (_) {
                return {};
              }
            })()
    ...
    // Extra slide IDs are written AFTER this bulky update so a large
    // filters/overlays payload cannot wipe or truncate them.
    if (Object.keys(editsPayload).length) {
      updateData.edits = JSON.stringify(editsPayload);
      needsUpdate = true;
    }
    if (form.link && form.link.trim() !== "") {
      updateData.link = form.link.trim();
    ...
    if (needsUpdate) {
      const updated = await safeUpdate(updateData);
      if (!updated) {
        const withoutEdits = { ...updateData };
        delete withoutEdits.edits;
        if (Object.keys(withoutEdits).length > 0) {
          await safeUpdate(withoutEdits);
        }
      }
    }

    if (extraPhotoFields.length) {
      const photosValue = extraPhotoFields.join(",");
      const savedPhotosField =
        (await safeUpdate({ photos: extraPhotoFields })) ||
        (await safeUpdate({ photos: photosValue }));
      void savedPhotosField;

      let currentEdits = updateData.edits || null;
      try {
        ...
        currentEdits = latest?.edits ?? currentEdits;
      } catch (_) {
        // Use in-memory edits if reload fails.
      }

      const mergedEdits = mergeExtraPhotoIdsIntoEdits(
        currentEdits,
        extraPhotoFields
      );
      const mergedOk = await safeUpdate({ edits: mergedEdits });
      if (!mergedOk) {
        await safeUpdate({
          edits: JSON.stringify({ p: extraPhotoFields }),
        });
      }
    }
    ...
    try {
      ...
      const hydrated = hydratePhotoPost(latest, { postType: "photo" });
      if (hydrated) return hydrated;
    } catch (_) {
      // Fall through to the original create result.
    }
    ...
    if (error.message && error.message.includes("Unknown attribute")) {
      throw new Error(`Database schema mismatch: ${error.message}. Please ensure your photo collection has at least these attributes: photo (string/url), creator (relationship to users), createdAt (datetime), createPhotoPost (datetime), visibility (enum). Optional: title, caption, filter, edits`);
    }
...
export async function getAllPhotoPosts() {
  try {
    ...
    return documents
      ...
      .map((post) => hydratePhotoPost(post, { postType: "photo" }))
      .filter(Boolean);
  } catch (error) {
    throw new Error(error);
  }
...
export async function getUserPhotos(userId) {
  try {
    ...
    return documents
      ...
      .map((photo) => hydratePhotoPost(photo))
      .filter(Boolean);
  } catch (error) {
    throw new Error(error);
  }
...
export async function getPhotoById(photoId) {
  try {
    ...
    const hydrated = hydratePhotoPost(photo);
    if (!hydrated) {
      throw new Error("Failed to fetch photo: missing photo file");
    }
    return hydrated;
  } catch (error) {
...
export async function toggleLike(userId, postId) {
  try {
    if (!userId || !postId) {
      throw new Error("userId and postId are required to toggle like");
    }
    ...
    if (isLiked) {
      ...
      // Add like - only include userId and postId (Appwrite automatically creates $createdAt)
    ...
    console.error('Toggle like error details:', {
      error: error.message,
      userId,
      postId,
      collectionId: appwriteConfig.likesCollectionId,
      fullError: error
    });
...
// Update user profile (username, avatar, privacy, bio, and birthday)
...
export async function toggleLikePost(postId, userId, postType = null) {
  ...
  try {
    ...
    if (error.message && error.message.includes("Unknown attribute")) {
      ...
      throw new Error(`The "${info.name}" (ID: ${info.id}) does not support likes. Please add the "likes" attribute (String array) to the collection schema in Appwrite.`);
    }
...
export async function toggleFollowUser(currentUserId, targetUserId) {
  ...
  if (isFollowing) {
    ...
    if (!Array.isArray(targetUser.profileLikes) || !targetUser.profileLikes.includes(currentUserId)) {
      try {
        await disableCreatorNotificationsOnUnfollow(targetUserId, currentUserId);
      } catch (_) {
    ...
    try {
      await enableCreatorNotificationsOnFollow(targetUserId, currentUserId);
    } catch (_) {
      /* subscription is best-effort */
    }
  }
...
/** All registered user IDs (paginated). Used for CEO-wide content broadcasts. */
export async function getAllPlatformUserIds(excludeUserId = null) {
  const ids = [];
  const pageSize = 100;
  let cursor = null;
  ...
  try {
    ...
    if (__DEV__) {
      console.warn('[notifications] getAllPlatformUserIds failed:', error?.message || error);
    }
  }
...
// Profile page likes count (stored on user.profileLikes[])
export async function getUserLikesCount(userId) {
  try {
    ...
    return Array.isArray(user.profileLikes) ? user.profileLikes.length : 0;
  } catch (error) {
    return 0;
  }
}

export async function isProfileLiked(currentUserId, targetUserId) {
  try {
    const user = await databases.getDocument(
      appwriteConfig.databaseId,
      appwriteConfig.userCollectionId,
    ...
    return Array.isArray(user.profileLikes) && user.profileLikes.includes(currentUserId);
  } catch (error) {
    return false;
  }
}
...
export async function getProfileLikers(userId) {
  try {
    ...
    const likerIds = user.profileLikes || [];
    const likers = [];

    for (const likerId of likerIds) {
      try {
        const liker = await databases.getDocument(
          appwriteConfig.databaseId,
          appwriteConfig.userCollectionId,
          likerId
        );
...
export async function toggleProfileLike(currentUserId, targetUserId) {
  if (!currentUserId || !targetUserId || currentUserId === targetUserId) {
    throw new Error('Invalid profile like action');
  }
  ...
  let profileLikes = Array.isArray(targetUser.profileLikes) ? [...targetUser.profileLikes] : [];
  const isLiked = profileLikes.includes(currentUserId);

  if (isLiked) {
    profileLikes = profileLikes.filter((id) => id !== currentUserId);

    try {
      const likeNotifications = await databases.listDocuments(
        appwriteConfig.databaseId,
        appwriteConfig.notificationsCollectionId,
        [
          Query.equal('type', 'profile_like'),
          Query.equal('fromUserId', currentUserId),
          Query.equal('targetUserId', targetUserId),
        ]
      );

      for (const notification of likeNotifications.documents) {
        try {
          await databases.deleteDocument(
            appwriteConfig.databaseId,
            appwriteConfig.notificationsCollectionId,
    ...
    if (!Array.isArray(targetUser.followers) || !targetUser.followers.includes(currentUserId)) {
      try {
        await disableCreatorNotificationsOnUnfollow(targetUserId, currentUserId);
      } catch (_) {
        /* subscription is best-effort */
    ...
    profileLikes.push(currentUserId);
    try {
      await createNotification('profile_like', currentUserId, targetUserId);
    } catch (error) {
      /* best-effort */
    }
    try {
  ...
  return { profileLikes, isLiked: !isLiked };
}

const DEDUPE_NOTIFICATION_TYPES = new Set([
  'follow',
  'profile_like',
  'live',
  'call',
...
async function findExistingNotification(type, fromUserId, targetUserId, postId = null) {
  const queries = [
...
export async function createNotification(type, fromUserId, targetUserId, postId = null) {
  try {
    if (DEDUPE_NOTIFICATION_TYPES.has(type)) {
    ...
    const pushTypes = new Set(['like', 'comment', 'follow', 'profile_like']);
    if (pushTypes.has(type)) {
      sendActivityPushNotification({
        type,
        fromUserId,
        fromUsername: fromUser.username,
...
export async function createAdvertisement(form) {
  try {
    ...
    // Optional video creative for in-feed playback
    if (form.video) {
      videoUrl = await uploadFile(form.video, "video");
      if (!videoUrl) {
        throw new Error("Failed to upload advertisement video");
      }
    }

    // Feed ads need playable media: video and/or image
    if (!imageUrl && !videoUrl) {
      throw new Error("Please add an image or video for your advertisement");
    }
    ...
    const basePayload = {
      ...
      // Prefer video URL in `image` when schema has no `video` attribute (feed plays it).
      // `#advideo` marker lets the feed treat Appwrite view URLs as video without a schema change.
      image: videoUrl
        ? `${videoUrl}${String(videoUrl).includes("#") ? "" : "#advideo"}`
        : imageUrl || "",
    ...
    // Prefer dedicated `video` attribute when present in Appwrite; fall back if missing
    if (videoUrl) {
      try {
        return await databases.createDocument(
          appwriteConfig.databaseId,
          ...
          ID.unique(),
          {
            ...basePayload,
            video: videoUrl,
            // Keep a poster in image when both exist
            image: imageUrl || `${videoUrl}${String(videoUrl).includes("#") ? "" : "#advideo"}`,
          }
        );
      } catch (attrError) {
        const msg = String(attrError?.message || attrError || "");
        if (!/unknown attribute|Invalid document structure|video/i.test(msg)) {
          throw attrError;
        }
      }
    }
