  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn('Could not get video file size:', sizeError.message);
      }
      ...
      console.log('✅ Video uploaded to storage:', uploadedVideo?.$id || 'unknown');
    } catch (uploadError) {
    ...
    if (music) {
      try {
        console.log('Uploading music to Appwrite Storage...');
        ...
        try {
          ...
          console.warn('Could not get music file size:', sizeError.message);
        }
        ...
        console.error('Music upload failed (optional):', musicError);
    ...
    try {
      ...
      console.log('✅ Function execution started:', execution?.$id || 'unknown');
    ...
    while (result.status === 'waiting' || result.status === 'processing') {
      ...
      try {
        ...
        console.log(`Execution status: ${result.status} (attempt ${attempts + 1})`);
      } catch (statusError) {
        console.error('Error checking execution status:', statusError);
    ...
    if (result.status === 'completed') {
      ...
      if (responseData.success && responseData.fileId) {
        ...
        console.error('Appwrite Function processing failed:', errorDetails);
    ...
    console.error('Appwrite Function video processing error:', error);
    throw error;
  }
}
...
export async function processVideoAuto({ 
  ...
  if (USE_APPWRITE_FUNCTIONS) {
    ...
    if (stickers.length > 0 || texts.length > 0 || videoClips || videoTransition !== 'none') {
      console.log('Advanced features detected, using server instead of Appwrite Functions');
...
export async function checkProcessingServer() {
  try {
    ...
    // Don't log error to avoid confusion - this is expected behavior
    return false;
  }
}
...
export async function checkAppwriteFunctions() {
  try {
    ...
    // Silently return false - don't log error to avoid confusion
    return false;
  }
}
